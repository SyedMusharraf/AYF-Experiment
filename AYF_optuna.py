...
def custom(y_true,y_pred):
    weights = tf.constant([5.0,5.0,100.0,5.0,5.0,50.0,5.0])
    sq_diff = tf.square(y_true-y_pred)
    weight_sq = sq_diff*weights
    return tf.reduce_mean(weight_sq)

def objeective(trial):
   model = Sequential()
   model.add(Dense(trial.suggest_int('units1',12,70,step=2),
                   activation='relu',input_shape=(X_train.shape[1],)))
   
    if trial.suggest_categorical('add_second_layer',[True,False]):
            model.add(Dense(trial.suggest_int('units2', 8, 64,step=2), activation='relu'))

    model.add(Dense(y_train.shape[1],))
    model.add(Lambda(lambda x:K.softplus(x)))

    learning_rate = trial.suggest_float('lr',1e-4,0.05,0.1,0.07,log= True)
    model.compile(optimizer=Adam(learning_rate=learning_rate),loss = custom, metrics=['mae'])
...
study.optimize(objective,n_trial = 30)
